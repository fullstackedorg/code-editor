// source: https://github.com/saminzadeh/codemirror-extension-inline-suggestion
...
const InlineSuggestionState = StateField.define<{ suggestion: null | string }>({
    ...
    update(s, tr) {
        ...
        if (!tr.docChanged && !inlineSuggestion && !tr.selection) {
            return s;
        }
        if (tr.state.doc)
            if (
...
function inlineSuggestionDecoration(view: EditorView, prefix: string) {
    ...
    const w = Decoration.widget({
        widget: new InlineSuggestionWidget(prefix, view),
        side: 1,
    });
...
class InlineSuggestionWidget extends WidgetType {
    suggestion: string;
    view: EditorView;
    constructor(suggestion: string, view: EditorView) {
        super();
